#!/bin/bash
# 测试 Python DSA 路径的 DeepSeek-V4 推理服务 (--model_impl python)
# 对比 native C++ 路径(run_dsv4_test.sh), 验证 Python DSA metadata 构建 + attention backend + 模型端到端
...
# Python model executor 需要能 import xllm.python
...
LOG_DIR=${LOG_DIR:-/mnt/sfs_turbo/tongpan/xllm/dsv4_py_logs}
...
# --- 3. 启动 8 进程(--model_impl python, 不开 graph 先跑通) ---
for i in $(seq 0 $((NNODES-1))); do
  ...
  ASCEND_RT_VISIBLE_DEVICES=$i nohup "$XLLM_PATH" \
    ...
    --dp_size=1 \
    > "$LOG_FILE" 2>&1 &
  echo "[$(date +%H:%M:%S)] started python node $i on port $PORT (device $i) -> $LOG_FILE"
done

echo "[$(date +%H:%M:%S)] 所有 $NNODES 个 Python 节点已启动..."
echo "监控: tail -f $LOG_DIR/node_0.log"
echo "成功标志: 'Brpc Server Started' / 'Application startup complete'"
