...
def compressor(
    ...
    """
    op_name = os.getenv("XLLM_DSV4_COMPRESSOR_OP", "compressor")
    if os.getenv("XLLM_DSV4_COMPRESSOR_DEBUG", "0") == "1":
        print(
            "[DSA COMPRESSOR]"
            f" op={op_name}"
            f" clone_state={os.getenv('XLLM_DSV4_COMPRESSOR_CLONE_STATE', '0')}"
            f" x={tuple(x.shape)} {x.dtype}"
            f" kv_state={tuple(kv_state.shape)} {kv_state.dtype}"
            f" score_state={tuple(score_state.shape)} {score_state.dtype}",
            flush=True,
        )
    if os.getenv("XLLM_DSV4_COMPRESSOR_CLONE_STATE", "0") == "1":
        kv_state = kv_state.clone()
        score_state = score_state.clone()
    meta_device = os.getenv("XLLM_DSV4_COMPRESSOR_META_DEVICE", "npu")
    move_meta_to_npu = meta_device == "npu" or (
        meta_device == "attn_npu" and kv_state.shape[-1] >= 512
    )
    if move_meta_to_npu:
        kv_block_table = (
            kv_block_table.to(x.device) if kv_block_table is not None else None
        )
        score_block_table = (
    ...
    op = (
        torch.ops.xllm_ops.compressor_noalias
        if op_name == "noalias"
        else torch.ops.xllm_ops.compressor
    )
    return op(
        x,
        wkv,
