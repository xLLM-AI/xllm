...
def chunk_kda_scaled_dot_kkt_fwd_kernel_intra_sub_inter(
    ...
    for i_k in range(tl.cdiv(K, BK)):
        ...
        m_k = o_k < K
        # [BK,]
        b_gn = tl.load(g + (i_t * BT + i_i * BC) * H * K + o_k, mask=m_k, other=0) * gk_scale
        # [BC, BK]
...
def precompile_k3_recompute_w_u_kernel(*, num_heads: int, dtype: torch.dtype, device: torch.device) -> bool:
...
def _get_k3_recompute_w_u_config(
    ...
    if not is_nvidia or gk is None or cu_seqlens is None or (K, V, BT) != (128, 128, 64):
        return None
...
def recompute_w_u_fwd(
    ...
    kernel = _recompute_w_u_fwd_kernel if static_config is not None else recompute_w_u_fwd_kernel
    kernel[(NT, B * H)](
        k=k,
        kg=kg,
        v=v,
