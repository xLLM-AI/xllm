...
class DsaMetadataBuilder:
    ...
    def build(
        ...
        dsa = self._build_seq_lengths(kv_seq_lens, q_lens)
    ...
    def _build_seq_lengths(
        ...
        max_kv = kv.max().to(torch.int32).reshape(1)
        max_q = q.max().to(torch.int32).reshape(1)
        max_query_len = int(max_q.item())
        max_seq_len = int(max_kv.item())
        return DsaMetadata(
            layer_id=-1,
            seq_lens=kv,
            seq_lens_q=q,
    ...
    def _build_positions(
        ...
        def _pad(positions: list[int], ratio: int) -> torch.Tensor:
            if enable_graph:
                ...
                # across bucket sizes.
                out = torch.full(
                    (max(total_tokens, 1),), -1, dtype=torch.int64
                )
            ...
            target = max(target, 1)
            out = torch.zeros(target, dtype=torch.int64)
            for idx, p in enumerate(positions):
                if idx >= target:
                    break
                out[idx] = p
    ...
    def _process_token_group(
        ...
        for seq in range(batch_size):
            ...
            q_len = min(int(q_lens[seq]), ctx_len)
    ...
    def _process_swa_group(
        ...
        for seq in range(batch_size):
            query_total_tokens += min(int(q_lens[seq]), int(ctx_lens[seq]))
        ...
        out_slots = torch.full(
            (max(out_slot_rows, 1),), -1, dtype=torch.int32, device=raw_bt.device
        )
        semantic_cols = int(raw_bt.size(1))
        ...
        new_bt = torch.full(
            (batch_size, max(max_dst_len, 1)), -1, dtype=torch.int32, device=raw_bt.device
        )
        for s in range(batch_size):
            if s >= raw_bt.size(0):
                continue
    ...
    def _expand_blocks_to_slots(
        ...
        slots = torch.full(
            (max(total_tokens, 1),), -1, dtype=torch.int32, device=raw_bt.device
        )
