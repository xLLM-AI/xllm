...
bool resolve_model_registration(const std::string& model_type,
  ...
  if (backend == kAutoBackend) {
    ...
    // qwen3_vl on TORCH is used by the Python model executor (--model_impl
    // python implements its own ViT + deepstack). qwen3_vl_moe has no Python
    // implementation yet, so it keeps its existing backend.
