...
// Out-of-line dtor: anchors the vtable to this .o so consumers that only
// see the forward declaration in xllm_server.h (via
// `unique_ptr<ModeSwitchService>`) still link cleanly against libscheduler.a
// where this impl lives, instead of each consumer TU emitting its own
// vtable-undef reference.
ModeSwitchService::~ModeSwitchService() = default;
