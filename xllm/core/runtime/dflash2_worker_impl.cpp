...
DFlashWorkerImpl::DraftBlock DFlash2WorkerImpl::run_decode_draft(
  ...
  BlockSampleOutput sampled;
  {
    ...
    sampled = sample_path(candidates, sampling_params, gumbel_noise);
  }

  DraftBlock draft_block;
  // DFlash2 samples selector paths from a sparse top-k distribution. The
  // release-branch DraftBlock carries selected-only probs [batch, steps]; the
  // shared validate path scatters them back to dense before the rejection
  // sampler, matching how every other draft algorithm feeds the verifier.
  // Greedy rows keep their one-hot semantics (selected prob == 1.0).
...
DFlash2WorkerImpl::BlockSampleOutput DFlash2WorkerImpl::sample_path(
    ...
    const torch::Tensor& gumbel_noise) const {
  ...
  if (sampling_params.temperatures.defined()) {
    // .to(kFloat32) aliases an already-fp32 input; clone before the in-place
    // temperature scaling so the const candidates stay untouched.
    edge_log_probs = edge_log_probs.clone();
  ...
  torch::Tensor selected_probs =
      torch::empty({batch_size, num_steps}, float_options);
  ...
  for (int64_t step = 0; step < num_steps; ++step) {
    ...
    // Selected-only proposal probs [batch, steps]: the value the shared
    // validate path needs is the sampled candidate's probability, so gather it
    // straight from the per-step top-k distribution instead of materializing
    // a [batch, steps, vocab] dense tensor on the decode hot path.
    selected_probs.index_put_(
        {ISlice(), step},
