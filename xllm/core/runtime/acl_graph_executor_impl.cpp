...
AclGraphExecutorImpl::AclGraphExecutorImpl(CausalLM* model,
  ...
  if (model_->supports_onerec_graph()) {
    onerec_graph_executor_ = std::make_unique<OneRecAclGraphExecutor>(
        model_, args_, device_, options_);
  }
}
...
ModelOutput AclGraphExecutorImpl::run(const torch::Tensor& tokens,
  ...
  if (onerec_graph_executor_ != nullptr &&
      onerec_graph_executor_->is_graph_candidate(params_single)) {
    return onerec_graph_executor_->run(
        tokens_tensor, positions_tensor, kv_caches, params_single);
  }
  if (onerec_graph_executor_ != nullptr) {
    onerec_graph_executor_->reset_first_prefill_graph_state_if_needed(
        params_single);
  }
