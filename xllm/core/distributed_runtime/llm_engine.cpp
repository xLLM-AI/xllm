...
void LLMEngine::rebuild_block_manager_pool(int32_t new_dp_size) {
  ...
  // Mirror init()'s pool-construction branching: host-offload / kvcache-store
  // flows use HierarchyBlockManagerPool (see the constructor call in init()
  // ~ line 634); everything else uses the flat BlockManagerPool. Upstream
  // #1861 re-enabled the hierarchy path with the LLMEngine-owned interface,
  // so the flip-time rebuild uses the same shape.
