...
torch::Tensor FusedMoEImpl::forward_expert(
  ...
  if (tp_pg_->world_size() > 1) {
    if (fc1_ctx && fc1_ctx->is_sequence_sharded()) {
      ...
      final_hidden_states = maybe_pad_and_reduce(
          final_hidden_states, ctx_copy, RowParallelReduceMode::REDUCE_SCATTER);
    } else {
  ...
  if (shared_output.has_value()) {
    ...
    if (tp_pg_->world_size() > 1) {
      if (fc1_ctx && fc1_ctx->is_sequence_sharded()) {
        FlashComm1Context ctx_copy = *fc1_ctx;
        ctx_copy.tp_group = tp_pg_;
        reduced_shared_output = maybe_pad_and_reduce(
...
torch::Tensor FusedMoEImpl::forward(const torch::Tensor& hidden_states,
  ...
  if (n_shared_experts_ > 0) {
    shared_output = fc1_ctx ? shared_experts_->forward(input, fc1_ctx)
                            : shared_experts_(input);
  ...
  auto output = forward_expert(input, router_logits, shared_output, fc1_ctx);
