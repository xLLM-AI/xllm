...
DSACPMetadata DSAMetadataBuilder::build_cp_local_metadata(
  ...
  if (num_reqs <= 0) {
    ...
    cp.local_kv_start_loc = torch::zeros({1}, int_cpu).to(in_dtype);
  } else {
    ...
    // Cumulative form of local_seq_lens (leading 0). This is the ori_kv
    // cu-seqlens: unlike local_query_start_loc it accounts for the full
    // right-aligned KV extent each local query sees, so the last rank's
    // entry sums to the full stream length rather than its local query count.
    auto local_kv_qsl = torch::zeros({num_reqs + 1}, int_cpu);
    local_kv_qsl.narrow(/*dim=*/0, /*start=*/1, /*length=*/num_reqs)
        .copy_(torch::cumsum(local_slens, /*dim=*/0));
    cp.local_kv_start_loc = local_kv_qsl.to(in_dtype);
  }
