...
torch::Tensor sample_gumbel_noise(int64_t batch_size,
  ...
  // Gumbel(0, 1) = -log(-log(U)), U ~ Uniform(0, 1).  torch::rand draws from
  // [0, 1) and can return exactly 0.0f, which maps log to -inf; the resulting
  // -inf gumbel value would then turn into NaN under the greedy-row zeroing
  // below (-inf * 0.0).  Clamp to the smallest positive normal so both the
  // random-row -inf and the mixed-batch NaN are impossible.
