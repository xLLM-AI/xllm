...
class Platform final {
 public:
  ...
  // MLU and ACL graph executors reuse decode graphs by padded token-row
  // buckets, so graph warmup must visit every batch size that starts a new
  // MTP token bucket. Other backends retain the compatibility warmup buckets
  // until their graph keying and MTP replay contracts are verified against
  // the same strategy.
