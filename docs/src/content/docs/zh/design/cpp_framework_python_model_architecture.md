...
已有 decode full CUDA graph 结果说明，capture/replay 可以显著减少这部分 host 开销。

这只能证明 decode graph 路径可行。当前 Python prefill graph 尚未完成，性能也不正常；
prefill、chunked prefill 和 mixed batch 仍需要后续实现并验证 graph 能力。因此 graph
全覆盖是目标架构达到极致性能的必要条件，不是当前已经完成的功能。
