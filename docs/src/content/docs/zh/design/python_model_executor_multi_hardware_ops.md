...
| 类别 | 注册位置 | 当前实现 | 新增硬件需做 |
|------|----------|----------|-------------|
| 计算 op (`rms_norm` / `fused_add_rms_norm` / `silu_and_mul` / `fused_qk_norm_rope`) | C++ `TORCH_LIBRARY(xllm_ops)` + `TORCH_LIBRARY_IMPL(CUDA)` | `core/kernels/cuda/xllm_ops_library.cpp` | 新增 `TORCH_LIBRARY_IMPL(xllm_ops, <DispatchKey>, m)` |
| KV cache write (`reshape_paged_cache`) | 同上 | 同上 | 同上 |
