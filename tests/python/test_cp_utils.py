...
# cp_utils imports ``from xllm.python import distributed`` for the CP all_gather
# at runtime; mock it so the module imports without the C++ binary. The tests
# emulate the collective directly instead of calling into distributed.
sys.modules.setdefault("xllm.python.distributed", MagicMock())
