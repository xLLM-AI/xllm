...
def test_vit_torchair_dynamic():
    ...
    Tests both single-shape and multi-shape execution:
    1. Single shape: 256x256 image (standard test)
    2. Multi-shape: sequentially execute [320, 1536] then [280, 1536] to verify
       dynamic shape support works correctly
    """
    # Part 1: Single shape test
    image_size = 256
    ...
    try:
        ...
        # Part 2: Multi-shape test
        # First input: 320 patches (16x20 grid)
        print("\n[TorchAir Dynamic Multi-Shape] Testing first input: [320, 1536] + [[1, 16, 20]]")
        pixel_values_1 = torch.randn(320, 1536, dtype=torch.bfloat16, device="npu")
        grid_thw_1 = torch.tensor([[1, 16, 20]], dtype=torch.int32, device="npu")
        
        with torch.no_grad():
            output_1 = vit_executor.execute(pixel_values_1, grid_thw_1)
        
        assert output_1 is not None
        assert output_1.shape[0] == 80  # 320 patches / 4 (2x2 merge)
        print(f"  First input succeeded: output_shape={output_1.shape}")
        
        # Second input: 280 patches (14x20 grid)
        print("\n[TorchAir Dynamic Multi-Shape] Testing second input: [280, 1536] + [[1, 14, 20]]")
        pixel_values_2 = torch.randn(280, 1536, dtype=torch.bfloat16, device="npu")
        grid_thw_2 = torch.tensor([[1, 14, 20]], dtype=torch.int32, device="npu")
        
        with torch.no_grad():
            output_2 = vit_executor.execute(pixel_values_2, grid_thw_2)
        
        assert output_2 is not None
        assert output_2.shape[0] == 70  # 280 patches / 4 (2x2 merge)
        print(f"  Second input succeeded: output_shape={output_2.shape}")
        print("\n[TorchAir Dynamic Multi-Shape] Multi-shape execution works!")
